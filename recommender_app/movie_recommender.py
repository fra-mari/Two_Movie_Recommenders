...
if __name__ == '__main__':
    ...
    try:
        ...
        print(Fore.BLUE+'Welcome to the Movie Recommender System developed by:\n\nBehzad Azarhoushang\nLaura Bartolini\nand Francesco Mari\n\nThis module will train a Non-Negative Matrix Factorization Model (NFM) on the MovieLensDataset and launch the webapp for interacting with it.\n'+Style.RESET_ALL)
        print(Fore.WHITE+Style.BRIGHT+'STEP 1: '+Style.NORMAL+'Traing the NMF model on the MovieLens dataset. The webapp will be launched as soon as the training is completed.\nThis will take a little time, though...'+Style.RESET_ALL)
        ...
        print(Fore.WHITE+Style.BRIGHT+'STEP 2: '+Style.NORMAL+'Now starting the Flask app run by the module `movies_app.py` IN THE BACKGROUND. In a few seconds, the website will be accessible at'+Fore.YELLOW+Style.BRIGHT+' http://localhost:5000 '+Fore.WHITE+Style.NORMAL+'in your browser.\n'+
        'The Log for the app may be found in the file'+Fore.YELLOW+Style.BRIGHT+' RecommenderLog.log'+Fore.WHITE+Style.NORMAL+'.'+Style.RESET_ALL)
        
        os.system('python movies_app.py &') 
        sleep(5)

        print(Fore.WHITE+'\nIf you wish, this module can now remain up to update the NMF Recommendation model every 12 hours.'+Style.RESET_ALL)
        keep_going = input(Fore.YELLOW+'Do you wish to keep the script up for cyclically updating the NMF model? Press any key to continue OR press "Q" to quit: '+Style.RESET_ALL)
        if keep_going == 'Q' or 'q':
            kill = input(Fore.BLUE+'Do you wish to kill both the NMF updater and the webapp? Press "Y" for killing both modules, or any other key for only killing the NMF updater: '+Style.RESET_ALL)
            if kill == 'Y' or 'y':
                os.system('pkill -9 -f movies_app.py')
                print('All right, the NMF updater and the webapp have been killed. Bye bye!')
                quit()
            else: 
                print('Quitting this module and the live update of the NMF.\n'+Fore.RED+'The webapp is still up and running! To shut it down, you should use the command:'+Fore.WHITE+' `pkill -9 -f movies_app.py`'+Fore.RED+' in your Terminal.\n'+Style.RESET_ALL)

        else: print(Fore.WHITE+'All right then. Keeping the NMF updater up. You can exit from it anytime with CTRL+C.\n'+Style.RESET_ALL)
        
        while True:
            
            sleep(60*60*12)

            logging.warning('Running the cyclical update of the NMF model based on the most recent recommandation provided by the website users...')
        ...
        print(' Quitting...\n'+Fore.RED+'N.B.: To quit this module will not stop the webapp if it is running! To shut it down, you should use the command:'+Fore.WHITE+' `pkill -9 -f movies_app.py`'+Fore.RED+' in your Terminal.\n'+Style.RESET_ALL)
